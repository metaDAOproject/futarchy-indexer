...
export type ErrorDetails = {
  ...
  [IndexTransactionError.VersionLookupTableMismatch]: {
    version: any;
...
const programToIndexer: Record<
  string,
  { indexer: InstructionIndexer<any>; coder: BorshCoder }
> = Object.fromEntries(
  indexers.map((indexer) => [
    indexer.PROGRAM_ID,
    { indexer, coder: new BorshCoder(indexer.PROGRAM_IDL) },
  ])
);

function error<E extends IndexTransactionError>(
  e: E,
  details: ErrorDetails[E]
): IndexTransactionResult<E> {
...
export async function indexTransaction(
  txIdx: number,
  signature: string
): Promise<IndexTransactionResult<any>> {
    ...
    case 0:
      ...
      for (const { accountKey } of transaction.message.addressTableLookups) {
        ...
        if (lookupTable === null) {
          ...
          return error(IndexTransactionError.InvalidLookupTable, {
      ...
      break;
    default:
      return error(IndexTransactionError.InvalidVersion, {
        version: tx.version,
      });
  }
  ...
  */
  ...
  const timeStr = new Date(tx.blockTime! * 1000)
    .toISOString()
    .replace("T", " ")
    .replace(".000Z", "");
    ...
    if (program in programToIndexer) {
      ...
      switch (secondLogMessage) {
        case "Program log: Instruction: IdlCreateAccount":
          console.log(
            yellow(`${prefix} skipping IdlCreateAccount ${signature}`)
          );
          continue;
      ...
      if (decoded == null) {
        console.log(
          `${prefix} Cannot decode instruction ${i} of transaction ${signature}`
        );
